<?php
function FillCombo($agregarPrimerItem = true, $maxItems = 0, $firstItem = "- SELECCIONAR -") {
	global $conn;
	global $RCfield;
	global $RCparams;
	global $RCquery;
	...
	if ($agregarPrimerItem)
		echo $RCwindow.".AddItemToDropDown('".$RCfield."', '-1', '".$firstItem."');";
	...
	while ($row = DBGetQuery($stmt)) {
		...
		echo $RCwindow.".AddItemToDropDown('".$RCfield."', '".$row["ID"]."', unescape('".rawurlencode($row["DETALLE"])."'));";
//		echo $RCwindow.".AddItemToDropDown('".$RCfield."', '".$row["ID"]."', unescape('".rawurlencode(htmlspecialchars_decode($row["DETALLE"], ENT_QUOTES))."'));";
		$index++;
	}
