function validarArchivo(value, exts) {
	value = value.toLowerCase();
	value = value.substr(value.lastIndexOf('.') + 1, 5);

	return (exts.indexOf(value) > -1);
}

function ValidarCampoTexto(field) {
  if (field.value == '') {
    alert('Por favor complete el campo ' + field.title + '.');
    if (field.parentDiv != undefined)
    	document.getElementById(field.parentDiv).style.display = 'block';
    field.focus();
    return false;
  }
...
function ValidarForm(form) {
	for (j=0; j<form.elements.length; j++) {
		if (form.elements[j].getAttribute('validar') == 'true') {
			if ((form.elements[j].type == 'file') || (form.elements[j].type == 'text') || (form.elements[j].type == 'textarea') || (form.elements[j].type == 'password')) {
				if (!ValidarCampoTexto(form.elements[j]))
					return false;
			}
			else if (form.elements[j].type == 'radio') {
				if (!ValidarRadioButton(form.elements[j]))
					return false;
			}
			else
		...
		if (form.elements[j].value != '') {
			if (form.elements[j].getAttribute('validarArchivo') == 'true')
				if (!validarArchivo(form.elements[j].value, form.elements[j].getAttribute('ext'))) {
					alert('El archivo seleccionado no tiene una extensión válida!');
					if (form.elements[j].parentDiv != undefined)
						document.getElementById(form.elements[j].parentDiv).style.display = 'block';
			...
			if (form.elements[j].getAttribute('validarPeriodo') == 'true')
				if (!validarPeriodo(form.elements[j].value, (form.elements[j].getAttribute('useSeparator') == 'true'))) {
...
function validarPeriodo(value, separator) {
	if (separator)
		if (value.substr(4, 1) != '/')
			return false;

	var ano = Number(value.substr(0, 4));
	if (separator)
		var mes = Number(value.substr(5, 2));
	else
		var mes = Number(value.substr(4, 2));
...
function ValidarRadioButton(field) {
