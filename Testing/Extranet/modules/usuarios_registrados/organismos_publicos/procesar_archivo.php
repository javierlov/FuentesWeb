...
try {
	...
	for ($row=2; $row<=$excel->rowcount(); $row++) {		// Empiezo desde la 2, porque en la 1 viene la cabecera..
		...
		$cols["G"] = str_replace(".", ",", (($cols["G"] == "")?0:$cols["G"]));
		$cols["U"] = str_replace(".", ",", (($cols["U"] == "")?0:$cols["U"]));
		$cols["V"] = str_replace(".", ",", (($cols["V"] == "")?0:$cols["V"]));
		$cols["W"] = str_replace(".", ",", (($cols["W"] == "")?0:$cols["W"]));
		$cols["X"] = str_replace(".", ",", (($cols["X"] == "")?0:$cols["X"]));
		$cols["Y"] = str_replace(".", ",", (($cols["Y"] == "")?0:$cols["Y"]));
		$cols["Z"] = str_replace(".", ",", (($cols["Z"] == "")?0:$cols["Z"]));
...
?>
			...
			if (confirm('Usted cargó una D.D.J.J. para el período <?= substr($periodo, 4, 2)."/".substr($periodo, 0, 4)?> el <?= $fechaDdjj?>.\n ¿ Desea eliminarla ?'))
				window.parent.location.href = '/index.php?pageid=46&page=paso2.php&id=<?= $seqTrans?>&amw=t&pp=<?= $periodo?>';		// amw=ActualizarMostrarWeb, pp=Período procesado..
			else
				window.parent.location.href = '/index.php?pageid=46&page=paso1.php';
<?
		}
		else {
?>
			window.parent.location.href = '/index.php?pageid=46&page=paso2.php&id=<?= $seqTrans?>';
<?
		}
?>
		</script>
...
//	echo "<script type='text/javascript'>alert(unescape('".rawurlencode($e->getMessage())."'));</script>";
	echo "<script type='text/javascript'>window.parent.location.href = '/index.php?pageid=46&page=paso3.php&id=".$seqTrans."&ep=t';</script>";
	exit;
}
?>