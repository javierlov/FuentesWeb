...
try {
	...
	if ($_POST["preguntasAdicionales"] == "t") {		// Guardo las preguntas adicionales..
		...
		if (intval($idSolicitudRGRL) < 1) {
			...
			$sql =
				"INSERT INTO hys.hsf_solicitudfgrl
										 (sf_fechaalta, sf_id, sf_idresolucionanexo, sf_idsolicitudestablecimiento, sf_usualta)
		...
		foreach ($_POST as $key => $value) {
			if (substr($key, 0, 9) == "pregunta_") {
				...
				$params = array(":idestablecimiento" => $_POST["idEstablecimiento"],
												...
												":usualta" => "W_".$_SESSION["usuario"]);
				$sql =
					"INSERT INTO hys.hra_respuestaadicional
											(ra_fechaalta, ra_id, ra_idpreguntaadicional, ra_idsolicitudestablecimiento, ra_respuesta, ra_usualta)
				...
				if (isset($_POST["Hplanilla_pregunta_".$_POST["H".$key]])) {		// Si la pregunta tiene planilla..
					...
					if ($idItem < 1) {		// Es un alta..
						$params = array(":idsolicitudfgrl" => $idSolicitudRGRL,
														...
														":usualta" => "W_".$_SESSION["usuario"]);
						$sql =
							"INSERT INTO hys.hsp_solicitudplanillafgrl
													 (sp_fechaalta, sp_id, sp_idsolicitudfgrl, sp_idtipoanexo, sp_usualta)
					...
					foreach ($_POST as $key2 => $value2) {
						if ((substr($key2, 0, 7) == "Hextra_") and (substr($key2, strpos($key2, "_pregunta_")) == "_pregunta_".$_POST["H".$key])) {
							...
							$params = array(":cumplimiento" => $cumplimiento,
															...
															":usualta" => "W_".$_SESSION["usuario"]);
							$sql =
								"INSERT INTO hys.hsi_solicituditemsplanillafgrl
														 (si_cumplimiento, si_fechaalta, si_id, si_iditemtipoanexo, si_idsolicitudplanillafgrl, si_usualta)
			...
			if (substr($key, 0, 9) == "pregunta_") {
				...
				if ($idItem < 1) {		// Es un alta..
					$params = array(":cumplimiento" => $value,
													...
													":usualta" => "W_".$_SESSION["usuario"]);
					$sql =
						"INSERT INTO hys.hst_solicituditemsfgrl
												 (st_cumplimiento, st_fechaalta, st_fecharegularizacion, st_id, st_iditem, st_idsolicitudfgrl, st_usualta)
