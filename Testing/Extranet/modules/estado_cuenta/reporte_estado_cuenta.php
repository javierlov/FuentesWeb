...
function getSqlDetalle($row, $sqlDatos) {
	...
	$sql = "SELECT legales.get_periodoconcoquiebra(:cuit) FROM DUAL";
	...
	$sqlVencimientoCuota = "deuda.get_vencimientocuota(SUBSTR('".sacarGuiones($row["CUIT"])."', 11, 1), rc_periodo)";
	...
	$sql = str_replace("#PRIMER_PERIODO_DEUDA#", $primerPeriodoDeuda, $sqlDatos).
		" FROM (SELECT ".$sqlVencimientoCuota." vencimiento,
										...
										cobranza.getsaldointereses(rc_contrato, rc_periodo) saldointereses, 0 extrajudicial,
										".$sqlTasaInteresFinal;
	if (!isFechaValida($maximaFechaConcursoQuiebra))
		$sql.= "'N' esconcursoquiebra";
	else
		$sql.= " UTILES.IIF_COMPARA('<=', RC_PERIODO, TO_DATE('".$maximaFechaConcursoQuiebra."', 'DD/MM/YYYY'), 'S', 'N') esconcursoquiebra";
	$sql.= " FROM zrc_resumencobranza
...
try {
	...
	$sqlFilas =
		"SELECT vencimiento,
						...
						contrato,
						prescripto,
						DECODE(esconcursoquiebra, 'S', 'Q') || DECODE(montorefinanciado, 0, '', 'R') || DECODE(importereclamoafip, 0, '', 'A') || DECODE(per_chequesrechazados, 'S', 'Z') ref";
	$sqlTotales =
	...
	$pdf = new FPDI("L", "mm", array(228, 280));
	...
	while ($rowDetalle = DBGetQuery($stmt, 1, false)) {
		...
		$pdf->Cell(17, 0, $rowDetalle["PAGOS"], 0, 0, "R");

		$pdf->Cell(24, 0, $rowDetalle["DEUDA_TOTAL"], 0, 0, "R");

		$pdf->Cell(18, 0, $rowDetalle["MONTOREFINANCIADO"], 0, 0, "R");

		$pdf->Cell(18, 0, $rowDetalle["IMPORTERECLAMOAFIP"], 0, 0, "R");

		$pdf->Cell(18, 0, $rowDetalle["INTERES"], 0, 0, "R");

		$pdf->Cell(20, 0, $rowDetalle["DEUDA_CONSOLIDADA"], 0, 0, "R");

		$pdf->Cell(3.5);
		$pdf->Cell(16, 0, $rowDetalle["REF"]);
	...
	$pdf->Cell(-2);
	$pdf->Cell(19, 0, $rowTotales["PAGOS"], 0, 0, "R");
	$pdf->Cell(24, 0, $rowTotales["DEUDA_TOTAL"], 0, 0, "R");
	$pdf->Cell(18, 0, $rowTotales["MONTOREFINANCIADO"], 0, 0, "R");
	$pdf->Cell(-2);
	$pdf->Cell(20, 0, $rowTotales["IMPORTERECLAMOAFIP"], 0, 0, "R");
	$pdf->Cell(-2);
	$pdf->Cell(20, 0, $rowTotales["INTERES"], 0, 0, "R");
	$pdf->Cell(-2);
	$pdf->Cell(22, 0, $rowTotales["DEUDA_CONSOLIDADA"], 0, 0, "R");
