		...
		<form action="/functions/telefonos/procesar_telefono.php" id="formTelefono" method="post" name="formTelefono" target="iframeProcesando">
			...
			<div style="margin-top:8px;">
				<div style="margin-bottom:2px; margin-left:0px;">
					...
					<select id="tipoTelefono" name="tipoTelefono"></select>
				</div>
				<div style="margin-bottom:2px; margin-left:68px;">
					<label class="ContenidoSeccion" for="area">Área</label>
					<input id="area" maxlength="5" name="area" style="margin-left:22px; width:48px;" type="text" value="<?= (!$isAlta)?$row["MP_AREA"]:""?>" />
				</div>
				<div style="margin-bottom:2px; margin-left:51px;">
					<label class="ContenidoSeccion" for="numero">Número (*)</label>
					<input id="numero" maxlength="60" name="numero" style="width:216px;" type="text" value="<?= (!$isAlta)?$row["MP_NUMERO"]:""?>" />
				</div>
				<div style="margin-bottom:2px; margin-left:54px;">
					<label class="ContenidoSeccion" for="interno">Interno</label>
					<input id="interno" maxlength="10" name="interno" style="margin-left:21px; width:88px;" type="text" value="<?= (!$isAlta)?$row["MP_INTERNO"]:""?>" />
				</div>
				<div style="margin-bottom:2px; margin-left:48px;">
					<label class="ContenidoSeccion" for="principal">Principal</label>
					<input <?= ((!$isAlta) and ($row["MP_PRINCIPAL"] == "S"))?"checked":""?> id="principal" name="principal" style="margin-left:21px; vertical-align:-3px;" title="Indica si es el teléfono principal para contactarse" type="checkbox" value="on" />
				</div>
				<div style="margin-bottom:2px; margin-left:12px;">
					<label class="ContenidoSeccion" for="observaciones">Observaciones</label>
					<input id="observaciones" maxlength="100" name="observaciones" style="margin-left:21px; width:216px;" type="text" value="<?= (!$isAlta)?$row["MP_OBSERVACION"]:""?>" />
				</div>
...
if (!$isAlta) {
?>
					<input class="btnDarDeBaja" style="margin-left:34px;" type="button" value="" onClick="eliminarTelefono(<?= $_REQUEST["idTelefono"]?>)" />
<?
}
?>
				</div>
...
$RCfield = "tipoTelefono";
$RCparams = array();
$RCquery =
	"SELECT tt_id id, tt_descripcion detalle
		 FROM att_tipotelefono
 ORDER BY 2";
$RCselectedItem = (!$isAlta)?$row["MP_IDTIPOTELEFONO"]:-1;
FillCombo();
?>
			document.getElementById('tipoTelefono').focus();
		</script>
	</body>
</html>