...
$sql1 =
	"SELECT ".addQuotes(getComputerAndUserName()).", pa_id, pa_idubicacion, pa_codigoppostal, pa_cpa,
...
$sql2 =
	"SELECT ".addQuotes(getComputerAndUserName()).", NULL, ub_id, ub_cpostal, NULL, ub_calle, 0, 9999, ub_localidad,
...
$sql =
	"INSERT INTO tmp_domicilios
							(mp_usuario, mp_idcodigospostales, mp_idubicacion, mp_cpostal, mp_cpa, mp_calle, mp_desde, mp_hasta, mp_localidad,
							 mp_provincia, mp_idprovincia, mp_partido) ".$sql1;
DBExecSql($conn, $sql, $params1, OCI_DEFAULT);

$sql =
	"INSERT INTO tmp_domicilios
							(mp_usuario, mp_idcodigospostales, mp_idubicacion, mp_cpostal, mp_cpa, mp_calle, mp_desde, mp_hasta, mp_localidad,
							 mp_provincia, mp_idprovincia, mp_partido) ".$sql2;
...
if ($grilla->recordCount() == 0) {
	$RCfield = "provincia2";
	$RCparams = array();
	$RCquery = "SELECT DISTINCT pv_codigo id, pv_descripcion detalle FROM cpv_provincias ORDER BY 2";
	$RCselectedItem = $_REQUEST["provincia"];
	FillCombo();
?>
