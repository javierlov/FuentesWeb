...
function editarNoticia(num) {
	window.location.href = '/index.php?pageid=64&idboletin=' + document.getElementById('id').value + '&num=' + num + '&&page=noticia.php';
}
...
function ordenar(id) {
	if ((divWin == null) || (divWin.style.display == 'none')) {
		...
		divWin = dhtmlwindow.open('divBox', 'iframe', '/test.php', 'Aviso', 'width=400px,height=264px,left=' + medioancho + 'px,top=' + medioalto + 'px,resize=1,scrolling=1');
	}

	divWin.load('iframe', '/modules/abm_arteria_noticias/orden.php?id=' + id, 'Ordenar noticias');
	divWin.show();
}

function setImagenPortada(imgName) {
	iframeBoletin.location = '/modules/abm_arteria_noticias/guardar_imagen_portada.php?id=' + document.getElementById('id').value + '&imgName=' + imgName;
}
...
function showTmpWin(id, tipo, titulo) {
	...
	divWin.load('iframe', '/modules/abm_arteria_noticias/campo_suelto.php?id=' + id + '&tipo=' + tipo + '&titulo=' + titulo, 'Indique el valor');
	divWin.show();
}

function volverAlBoletin() {
	window.location.href = '/index.php?pageid=64&page=boletin.php&id=' + document.getElementById('idboletin').value;
}