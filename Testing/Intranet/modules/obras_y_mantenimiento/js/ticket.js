...
function AddAttachment(idelement) {
  ...
  if ($inputLibre) {
    AjaxRequest('DivAreaMensajes', 'ajax_mensajes.php', 'Todavía tiene una casilla libre para adjuntar un archivo.');
    return false;
  } else {
...
function ValidarFormCalificacion() {
  ...
  if ((document.getElementById('resuelto').value != 'N') &&(document.getElementById('calificacion').value == -1)) {
    AjaxRequest('DivAreaMensajes', 'ajax_mensajes.php', 'Por favor indique que calificación nos daría por la resolución de este pedido.');
    document.getElementById('calificacion').focus();
    return false;
  }
  if ((document.getElementById('resuelto').value == 'N') && (document.getElementById('calificacion').value > -1)) {
    AjaxRequest('DivAreaMensajes', 'ajax_mensajes.php', 'Si el ticket no fue resuelto, nos pondremos a revisar el motivo. Pero aún no es tiempo de que nos califique, deje el campo calificación en blanco.');
    document.getElementById('calificacion').focus();
    return false;
  }
  if (((document.getElementById('calificacion').value == 5) || (document.getElementById('calificacion').value == 6)) && (document.getElementById('comentarios').value == '')) {
    AjaxRequest('DivAreaMensajes', 'ajax_mensajes.php', 'Nos gustaría que si su calificación es regular o mala nos indique el motivo.');
    document.getElementById('comentarios').focus();
    return false;
  }
...
function ValidarFormInformacion() {
  if (document.getElementById('comentarios').value == '') {
    AjaxRequest('DivAreaMensajes', 'ajax_mensajes.php', 'Sería bueno que nos aporte mas información sobre lo solicitado.');
    document.getElementById('comentarios').focus();
    return false;
  }
...
function ValidarFormTicket() {
    ...
    AjaxRequest('DivAreaMensajes', 'ajax_mensajes.php', 'Debe seleccionar la aplicación del Portal que corresponda.');
    document.getElementById('Ejecutable').focus();
    return false;
  }
	if (document.getElementById('notas').value == '') {
		AjaxRequest('DivAreaMensajes', 'ajax_mensajes.php', 'Indique una breve descripción de su pedido.');
    document.getElementById('notas').focus();
    return false;
  }
	if (document.getElementById('notas').value.length > 1000) {
		AjaxRequest('DivAreaMensajes', 'ajax_mensajes.php', 'La descripción de su pedido no puede superar los 1000 caracteres.');
    document.getElementById('notas').focus();
    return false;
  }
	...
	return ValidarForm(formSolicitud);
}

function ValidarPermisoUsuario() {
  if((document.formSolicitud.UsuarioSolicitud.selectedIndex > -1)
   & (document.formSolicitud.DetallePedido.selectedIndex > -1)) {
    if ((document.formSolicitud.UsuarioSolicitud.options[document.formSolicitud.UsuarioSolicitud.selectedIndex].value > 0)
       & (document.formSolicitud.DetallePedido.options[document.formSolicitud.DetallePedido.selectedIndex].value > 0)) {
      AjaxRequest('DivAreaMensajes', 'ajax_mensaje_permiso.php',
                  document.formSolicitud.UsuarioSolicitud.options[document.formSolicitud.UsuarioSolicitud.selectedIndex].value,
                  document.formSolicitud.DetallePedido.options[document.formSolicitud.DetallePedido.selectedIndex].value);
    }
  }
  return 1;
}