...
/*
Clase preparada para dibujar una grilla pasando un query de ODBC como parámetro..
...
class Grid_ODBC extends Grid {
	...
	private function DrawFooter() {
		...
		if ($this->GetColBaja() != -1) {		// Si está configurada cual es la columna de baja..
			...
			$url = $this->BuildUrl(1);
			if ($this->showBajas)
				$this->code.= '<img border="0" src="/images/grid_ocultar_bajas.gif" style="cursor:pointer" title="Ocultar registros dados de baja" onClick="changePage(\''.$url.'\', '.$showMessage.')" />';
			else {
				$this->code.= '<img border="0" src="/images/grid_ver_bajas.gif" style="cursor:pointer" title="Ver registros dados de baja" onClick="changePage(\''.$url.'\', '.$showMessage.')" />';
			}
		}
	...
	private function DrawRows($colOpenIfOneRecord) {
		...
		$countRecords = 1;
		while ($row = DBGetQuery($this->stmt)) {
					...
					if ($this->columns[$i]->getButtonClass($row) != "") {
						if ((!$this->showButtonDespiteBaja) and ($this->GetColBaja() != -1) and ($row[$this->GetColBaja()] != ""))
							...
							if ($hideCell) {		// Si hay que ocultar el contenido de esta celda no muestro el botón..
								...
								$input = '<input class="'.$this->columns[$i]->getButtonClass($row).'" '.$title.' type="button" onClick="'.$msgEspera.' '.$url.'" />';
							}
