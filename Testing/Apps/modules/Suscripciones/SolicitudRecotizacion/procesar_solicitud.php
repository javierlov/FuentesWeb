	...
	if ($_REQUEST["PERMITE"] == "S") {
		...
		$sql =
			"UPDATE asc_solicitudcotizacion
					SET (sc_codmotivorevision, sc_edadpromedio, sc_establecimientos, sc_estado, sc_fecharevision,
							 sc_idprobabilidadcierre, sc_motivorevision, sc_porcaumento, sc_porcdescuento, sc_sector,
							 sc_usuariorevision, sc_usuariosolicitud) =
			 (SELECT ae_codmotivorecotizacion, ae_edadpromedio, ae_establecimientos, :estado, SYSDATE,
							 ae_idprobabilidadcierre, ae_observaciones, NULL, NULL, ae_sector,
							 :usuario, :usuario
				 FROM afi.aae_autorizarrecotizacion
			  WHERE ae_idtransaccionweb = :idtransaccion)
			  WHERE sc_id = :id";
