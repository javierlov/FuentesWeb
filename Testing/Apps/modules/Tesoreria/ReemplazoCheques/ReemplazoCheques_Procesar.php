	...
	foreach($_REQUEST as $nombre => $valor)		// En este foreach inserto los nuevos beneficiarios menos el primero..
		if (substr($nombre, 0, 19) == "NUEVO_BENEFICIARIO_") {
			$params = array(":beneficiario" => substr($valor, 0, 100),
											...
											":usurespuesta" => $_REQUEST["USERNAME"]);
			$sql = 
				"INSERT INTO rrb_reemplazobeneficiario (rb_id, rb_idchequeemitido, rb_fechaalta, rb_usualta,
																								rb_fecharespuesta, rb_usurespuesta, rb_beneficiario,
																								rb_idtransaccionweb)
																				VALUES (SEQ_RRB_ID.NEXTVAL, :idchequeemitido, ACTUALDATE, :usualta,
																								SYSDATE, :usurespuesta, UPPER(:beneficiario),
																								:idtransaccion)";
			DBExecSql($conn, $sql, $params);
		}
