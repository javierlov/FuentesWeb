...
if ($paso == 2) {
	if ($_FILES["imagen"]["name"] == "") {
		echo "<script>alert('Debe seleccionar una imagen.'); history.back();</script>";
		exit;
	}

	if (!validarExtension($_FILES["imagen"]["name"])) {
		echo "<script>alert('Debe seleccionar una imagen válida.'); history.back();</script>";
		exit;
	}
					...
					if (this.curCrop == null) {
						this.curCrop = new Cropper.Img(
							'testImage',
							{
								...
								<?= ($_REQUEST["mantenerProporcion"] == "t")?"ratioDim: {x: ".$_REQUEST["minWidth"].", y: ".$_REQUEST["minHeight"]."},":"" ?>
								onEndCrop: onEndCrop
							}
						);
					} else {
...
?>
		<style type="text/css">
			...
			#btnGuardar {cursor:pointer; margin-bottom:16px;}
			#btnFinalizar {cursor:pointer; margin-bottom:16px;}
			#dimensionsForm {float:right; width:350px;}
			#testWrap {margin:20px 0 40px 50px; /* Just while testing, to make sure we return the correct positions for the image & not the window */}
		</style>
	</head>
	<body bgcolor="#eeeeee">
		<p style="margin-left: 12px"><img src="/images/header_editor_imagenes.gif"></p>
		<span style="font-family:Arial; font-size:9pt; color:#000000; margin-left:30px;">
<?
if ($paso == 1) {
?>
...
?>
		...
		<div id="testWrap">
			<div style="float:left; margin-left:-20px; margin-right:16px; margin-top:3px;">
				<p style="<?= (($paso == 1)?"background-color:#00a4e4; border:1px solid #807f84; color:#fff; font-family:Arial; font-size:9pt;":"border:1px solid #00a4e4; color:#8c8c8c; background-color:#fff; font-family:Arial; font-size:9pt;")?> cursor:default;">
					&nbsp;Paso 1&nbsp;
				</p>
				<p style="<?= (($paso == 2)?"background-color:#00a4e4; border:1px solid #807f84; color:#fff; font-family:Arial; font-size:9pt;":"border:1px solid #00a4e4; color:#8c8c8c; background-color:#fff; font-family:Arial; font-size:9pt;")?> cursor:default;">
					&nbsp;Paso 2&nbsp;
				</p>
				<p style="<?= (($paso == 3)?"background-color:#00a4e4; border:1px solid #807f84; color:#fff; font-family:Arial; font-size:9pt;":"border:1px solid #00a4e4; color:#8c8c8c; background-color:#fff; font-family:Arial; font-size:9pt;")?> cursor:default;">
					&nbsp;Paso 3&nbsp;
				</p>
			</div>
			<div id="#paso1" style="display:<?= (($paso == 1)?"block":"none")?>;">
				<form action="<?= $_SERVER["PHP_SELF"]?>" enctype="multipart/form-data" id="formImagen1" method="post" name="formImagen1">
					...
					<input id="mantenerProporcion" name="mantenerProporcion" type="hidden" value="<?= (isset($_REQUEST["mantenerProporcion"]))?"t":"f"?>" />
					<input id="paso" name="paso" type="hidden" value="2">
					<input id="imagen" name="imagen" style="cursor:pointer;" type="file">
					<input id="btnSubirImagen" style="cursor:pointer; margin-left:24px;" type="submit" value="Subir imagen" />
				</form>
			</div>
			<div id="#paso2" style="display:<?= (($paso == 2)?"block":"none")?>; float:left;">
			...
			<div id="#paso3" style="display:<?= (($paso == 3)?"block":"none")?>;">
				<form action="<?= $_SERVER["PHP_SELF"]?>" id="formImagen3" method="post" name="formImagen3" onSubmit="return ValidarForm(formImagen3)">
					...
					<input id="btnFinalizar" type="submit" value="Finalizar" />
				</form>
				<img id="testImage" src="<?= "/functions/get_image.php?file=".base64_encode($file)?>" />
			</div>
		</div>
