...
if (!isset($_REQUEST["firstcall"]))
  ...
  if (isset($_REQUEST["close_button"]))
    ...
    $close_button = "yes";
?>
<form action="/modules/gestion_sistemas/index.php?sistema=<?echo $sistema;?>&search=yes&all_tickets=yes&firstcall=false" id="formTicket" method="post" name="formTicket" onSubmit="ValidarForm(formTicket)">
	<span style="left:0px; position:relative;">
		<label class="FormLabelBlanco" for="numeroTicket">Nº Ticket</label>
		<input id="numeroTicket" name="numeroTicket" size="10" title="Nº Ticket" type="text" validarEntero="true" value="<?= $numeroTicket?>" style="background-color: #ccc; border: 1px solid #808080; color: #000; font-family: Arial; font-size: 10pt; margin-right:8px;" />
		<input  id="btnBuscar" name="btnBuscar" type="submit" value="Buscar" style="background-color: #808080; border: 1px solid #333; color: #fff; cursor: pointer; font-family: Arial; font-size: 8pt;" />
	</span>
</form>
...
*/
  ...
  $grilla = new Grid(array("", "N° de Ticket", "Fecha", "Motivo", "Detalle", "Estado", "Pedido por", ""),
	...
	$grilla = new Grid();
	$grilla->addColumn(new Column("", 8, true, false, 9, "btnTicket", "index.php?sistema=".$sistema."&ticket_detail=yes&amp;all_tickets=".$all_tickets."&amp;pending_tickets=".$pending_tickets."&amp;back_button=".$back_button."&amp;close_button=".$close_button, "GridFirstColumn"));
