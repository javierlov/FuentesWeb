...
$sql =
	"SELECT eu_proyecto
		 FROM web.weu_encuesta_usuario
		WHERE eu_id = :id";
$proyecto = valorSql($sql, "", $params);

$params = array(":anio" => $periodo);
$sql =
...
<script type="text/javascript">
	with (window.parent.document) {
		...
		getElementById('hProyecto').innerText = 'PROYECTO: <?= $proyecto?>';
...
while ($row = DBGetQuery($stmt)) {
	...
	$sql =
		"SELECT em_calificacion, REPLACE(REPLACE(em_observaciones, CHR(10), ''), CHR(13), '') em_observaciones
...
?>
		...
		str+= '<td align="center" style="background-color:#99ccff;"><?= ($primerRegistro)?"Calificación":""?></td>';
...
?>
		...
		str+= '<td align="center"><input id="promedioTotal" name="promedioTotal" readonly style="background-color:#8db3e2; font-size:20px; width:40px;" type="text" value="" /></td>';
