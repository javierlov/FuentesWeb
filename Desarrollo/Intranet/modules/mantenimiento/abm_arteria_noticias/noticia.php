...
$sql =
	"SELECT na_altoimagenes, na_anchoimagenes, na_colortitulo, na_id, na_nota, na_numeroplantilla, na_titulo, na_visible
...
<div align="left" id="divPanelAbm" style="background-color:#eee;">
	<form action="/modules/mantenimiento/abm_arteria_noticias/procesar_noticia.php" enctype="multipart/form-data" id="formNoticia" method="post" name="formNoticia" target="iframeNoticia">
		...
		<p style="margin-left:13px;">
			<label class="FormLabelAzul" for="visible" style="margin-right:4px; vertical-align:3px;">Visible</label>
			<input <?= (($_REQUEST["num"] < 5) or ($row["NA_VISIBLE"] == "S"))?"checked":""?> <?= ($_REQUEST["num"] < 5)?"disabled":""?> id="visible" name="visible" type="checkbox" value="ok" />
			<img src="/modules/mantenimiento/abm_arteria_noticias/images/ocultar.png" style="cursor:pointer; margin-left:656px; margin-top:4px;" title="Clic aquí para ocultar el panel de edición" onClick="ocultarPanelAbm()" />
		</p>
		...
		<p style="margin-left:15px; margin-top:4px;">
			...
			<select class="Combo" id="fondo" name="fondo" onChange="cambiaFondo()">
				<option value="-1" <?= ($row["NA_COLORTITULO"] == "")?"selected":""?>>- SELECCIONAR -</option>
<?
...
?>
			...
			<img id="imgFondoChica" src="/modules/arteria_noticias/fondo_titulos/fondo_chico_00ADEF.jpg" style="cursor:pointer; height:13px; margin-left:16px; margin-right:16px; visibility:hidden; width:90px;" title="Clic aquí para redimensionar la imagen" onClick="ajustarImagen(this, '13px', '90px')" />
			<img id="imgFondoGrande" src="/modules/arteria_noticias/fondo_titulos/fondo_grande_00ADEF.jpg" style="cursor:pointer; height:21px; visibility:hidden; width:380px;" title="Clic aquí para redimensionar la imagen" onClick="ajustarImagen(this, '21px', '380px')" />
		</p>
...
if ($row["NA_ID"] == "") {
?>
				<img alt="Antes de agregar una imagen debe guardar la noticia por primera vez" src="/modules/mantenimiento/abm_arteria_noticias/images/imagen_lockeada.png" style="vertical-align:middle;" />
<?
}
else {
?>
<!--
			<a href="/functions/edit_image.php?finalFunction=addImagen&minWidth=<?= $row["NA_ANCHOIMAGENES"]?>&maxWidth=<?= $row["NA_ANCHOIMAGENES"]?>&minHeight=<?= $row["NA_ALTOIMAGENES"]?>&maxHeight=<?= $row["NA_ALTOIMAGENES"]?>" target="_blank">
				<img alt="Agregar imagen" src="/modules/mantenimiento/abm_arteria_noticias/images/agregar_imagen.png" style="cursor:pointer; vertical-align:middle;" />
			</a>
-->
			<div style="left:104px; position:relative; top:-16px;">
				<div style="float:left; height:26px; padding-top:4px;">Imagen</div>
				<div style="float:left; margin-left:8px;">
					<iframe frameborder="0" id="iframeSubirImagen" name="iframeSubirImagen" scrolling="no" src="/modules/mantenimiento/abm_arteria_noticias/seleccionar_imagen.php" style="height:26px; width:400px;"></iframe>
				</div>
			</div>
<?
}
...
if ($row["NA_ID"] != "") {
?>
		<iframe frameborder="1" id="iframeImagenes" name="iframeImagenes" scrolling="yes" src="/modules/mantenimiento/abm_arteria_noticias/imagenes.php?idnoticia=<?= $row["NA_ID"]?>" style="height:120px; margin-left:68px; width:640px;"></iframe>
<?
}
?>
...
<table cellpadding="0" cellspacing="0" align="center" width="745">
	<tr><td colspan="2"><img src="/modules/arteria_noticias/images/header.jpg" /></td></tr>
