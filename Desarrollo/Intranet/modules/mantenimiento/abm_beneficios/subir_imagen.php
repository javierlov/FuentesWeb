...
try {
	...
	if (!in_array($partes_ruta["extension"], $extensionesPermitidas))
		...
		elseif (filesize($tmpfile) > $maxFileSize)
			...
			$filename = IMAGES_BENEFICIOS_PATH.$_POST["carpeta"]."\\".$partes_ruta["filename"].".".$partes_ruta["extension"];
			...
			while (file_exists($filename)) {
				...
				$filename = IMAGES_BENEFICIOS_PATH.$_POST["carpeta"]."\\".$partes_ruta["filename"]."_".$i.".".$partes_ruta["extension"];
			}

			if ($msgError == "") {		// Si pudimos obtener el nombre con el que va a quedar el archivo en el servidor..
				if (!makeDirectory(IMAGES_BENEFICIOS_PATH.$_POST["carpeta"]))
