...
class Grid {
	...
	private $sortByCol = true;		// Indica si se puede hacer clic en el nombre de la columna para reordenar la grilla o no..
	private $stmt;
			...
			if ($col->getVisible()) {		// Si la columna está visible..
				...
				if ($this->orderBy == $i) {
					...
					if (!strpos($this->orderBy, "_D_")) {
						if ($i > 0)
							$img = '<img src="/images/grilla/grid_asc.gif" title="Ascendente" />';
						$url = $this->BuildUrl(1, $i."_D_");
					}
					else {
						if ($i > 0)
							$img = '<img src="/images/grilla/grid_desc.gif" title="Descendente" />';
				...
				if ($this->sortByCol)
					...
					$this->code.= '<td align="center" class="'.$this->titleStyle.'" '.$title.'>'.$col->getTitle().'</td>';
	...
	private function drawFooter() {
		...
		if ($this->getColBaja() != -1) {		// Si está configurada cual es la columna de baja..
			...
			$url = $this->BuildUrl(1);
			if ($this->showBajas)
				$this->code.= '<img src="/images/grilla/grid_ocultar_bajas.gif" style="cursor:pointer" title="Ocultar registros dados de baja" onClick="changePage(\''.$url.'\', '.$showMessage.', '.(($this->refreshIntoWindow)?'true':'false').')" />';
			else {
				$this->code.= '<img src="/images/grilla/grid_ver_bajas.gif" style="cursor:pointer" title="Ver registros dados de baja" onClick="changePage(\''.$url.'\', '.$showMessage.', '.(($this->refreshIntoWindow)?'true':'false').')" />';
			}
		}
	...
	private function drawRows($colOpenIfOneRecord) {
				...
				if ($this->columns[$i]->getVisible()) {		// Si la columna está visible..
					...
					if ($this->columns[$i]->getButtonClass($row) != "") {
						if ((!$this->showButtonDespiteBaja) and ($this->getColBaja() != -1) and ($row[$this->getColBaja()] != ""))
							...
							if ($this->columns[$i]->getUrlAmigable())		// Si es un url amigable..
								$parametroId = "/".$value;
							elseif (strpos($this->columns[$i]->getActionButton(), ".") === false)		// Si no se llama a un archivo directamente..
								$parametroId = $value;
							elseif (strrpos($this->columns[$i]->getActionButton(), "?") == 0)		// Si la url no tiene parámetros..
								$parametroId = "?id=".$value;
							else		// Sino, si tiene parámetros..
								$parametroId = "&id=".$value;
							...
							if (($this->getColBaja() != -1) and ($row[$this->getColBaja()] != ""))		// Si la columna de baja está seteada y es una baja..
								$this->code.= '<td align="center" class="'.$this->columns[$i]->getCellClass().' '.$this->bajaStyle.'" style="'.$colStyle.' '.$css.'">'.$input.'</td>';
							else
						...
						if (($this->getColBaja() != -1) and ($row[$this->getColBaja()] != ""))		// Si la columna de baja está seteada y es una baja..
							$this->code.= '<td align="left" class="'.$this->columns[$i]->getCellClass().' '.$this->textStyle.' '.$this->bajaStyle.'" style="'.$colStyle.' '.$css.'"><div class="'.$this->bajaStyle.'" '.$title.'>'.$value.'</div></td>';
						else
		...
		// Si no mostró ningún registro es porque quedo configurado en una página que no existe para el sql actual, asi que llamo a la página 1..
		if ($countRecords == 0) {
	...
	public function setSortByCol($value) {
		$this->sortByCol = $value;
	}
