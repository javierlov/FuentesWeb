...
{	
	...
	$this->PageFormats=array('a3'=>array(841.89,1190.55), 'a4'=>array(595.28,841.89), 'a5'=>array(420.94,595.28),
		'letter'=>array(612,792), 'legal'=>array(612,1008), 'legaldouble'=>array(1224,2016));
	if(is_string($format))
...
function Cell($w, $h=0, $txt="", $border=0, $ln=0, $align="", $fill=false, $link="", $maxLengthActived=true) {
	// INICIO - Corto el texto al tamaño de la celda (27.5.2011)..
	if (($w > 0) and (strlen($txt) > 0) and ($maxLengthActived)) {
		if ($txt == strtoupper($txt))
			$wTmp = $w - 2;
		else
			$wTmp = $w + 2;

		while ($this->GetStringWidth($txt) > $wTmp)
			$txt = substr($txt, 0, -1);
	}
	// FIN - Corto el texto al tamaño de la celda (27.5.2011)..
