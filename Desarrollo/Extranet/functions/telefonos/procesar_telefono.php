...
function validar() {
	...
	if ($_POST["area"] == "") {
		echo "errores+= '- El campo Área es obligatorio.<br />';";
		$errores = true;
	}
	...
	if ((substr($_POST["area"], 0, 1) != "0") or (substr($_POST["area"], 0, 2) == "00")) {
		echo "errores+= '- El Área debe comenzar con \"0\" pero no con \"00\".<br />';";
		$errores = true;
	}

	if ((strlen($_POST["area"]) < 3) or (strlen($_POST["area"]) > 5)) {
		echo "errores+= '- El Área debe tener entre 3 y 5 dígitos.<br />';";
		$errores = true;
	}
	...
	if (substr($_POST["numero"], 0, 1) == "0") {
		echo "errores+= '- El campo Número no puede comenzar con cero \"0\".<br />';";
		$errores = true;
	}
...
<script type="text/javascript">
	function redirect() {
		...
		// Agrego el 1 por las dudas..
		obj = window.parent.parent.document.getElementById('iframeTelefonos1');
		if (obj != null)
			obj.contentWindow.location.reload(true);
