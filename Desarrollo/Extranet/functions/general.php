...
function getFile($file, $mode = "i") {
	...
	return "/archivo/".$str."/".$mode."/".date("Ymdhisu");
}
...
function limpiarXss() {
	if (isset($_GET))
		foreach ($_GET as $key => $value)
			$_GET[$key] = htmlspecialchars($value, ENT_QUOTES, CHARSET);

	if (isset($_POST))
		foreach ($_POST as $key => $value)
			$_POST[$key] = htmlspecialchars($value, ENT_QUOTES, CHARSET);

	if (isset($_REQUEST))
		foreach ($_REQUEST as $key => $value)
			$_REQUEST[$key] = htmlspecialchars($value, ENT_QUOTES, CHARSET);
}
...
function logAccess($usuario, $idTipoUsuario, $remoteHost, $remoteIp, $idPagina) {
	...
	$sql =
		"INSERT INTO web.wle_logextranet (le_usuario, le_idtipousuario, le_remotehost, le_remoteip, le_fechaacceso, le_idpagina)
...
function validarAccesoCotizacion($idModulo) {
// Se valida que el que quiera ver una cotización, afiliación o algún dato relativo a esos dos, sea del mismo canal - entidad - sucursal - vendedor..
	...
	$id = intval(substr($idModulo, 1));
	$modulo = substr($idModulo, 0, 1);
...
function validarPermisoClienteXModulo($idCliente, $modulo) {
	if ((isset($_SESSION["isCliente"])) and (!$_SESSION["isAdminTotal"]) and (!$_SESSION["contratoVigente"]) and ($modulo != 64))		// Este IF está hecho por el ticket 59724..
		return false;
...
function validarSesion($isValid) {
	if (!$isValid) {
		...
		echo "<script>if (!isTopLevel(window)) alert('Su sesión ha expirado.\\nPor favor logueese nuevamente para continuar.');</script>";
		echo "<span id=\"sesionInvalidData\">".$_SERVER["REMOTE_ADDR"]." (".gethostbyaddr($_SERVER['REMOTE_ADDR']).")</span><br />";
		echo "<span id=\"sesionInvalidMsg\">Su sesión ha expirado. Por favor logueese nuevamente para continuar.";
		exit;
	}
}

function validarSSL() {
	if (((isset($_REQUEST["pageid"])) and ($_REQUEST["pageid"] == 25)) or
			((isset($_REQUEST["pageid"])) and ($_REQUEST["pageid"] == 31)) or
			((isset($_REQUEST["pageid"])) and ($_REQUEST["pageid"] == 43)) or
			((isset($_REQUEST["pageid"])) and ($_REQUEST["pageid"] == 45)) or
			((isset($_REQUEST["pageid"])) and ($_REQUEST["pageid"] == 49)) or
			((isset($_SESSION["isAbogado"])) and ($_SESSION["isAbogado"])) or
			((isset($_SESSION["isAgenteComercial"])) and ($_SESSION["isAgenteComercial"])) or
			((isset($_SESSION["isCliente"])) and ($_SESSION["isCliente"])) or
			((isset($_SESSION["isOrganismoPublico"])) and ($_SESSION["isOrganismoPublico"])) or
			((isset($_SESSION["isPreventor"])) and ($_SESSION["isPreventor"]))) {
