...
function validar() {
	...
	if ($_POST["cuit"] == "") {
		echo "errores+= '- El campo CUIT es obligatorio.<br />';";
		$errores = true;
	}
	
	if ($_POST["establecimiento"] == -1) {
		echo "errores+= '- El Establecimiento debe seleccionarse.<br />';";
		$errores = true;
	}
	...
	$sql = "SELECT HYS.SEQ_HVP_VISITAPREVENTOR.NEXTVAL IDVISITA FROM DUAL";
	$stmt = DBExecSql($conn, $sql);
	$row = DBGetQuery($stmt);
	...
	$curs = null;
	$params = array(":cuit" => $_POST["cuit"],":establecimiento" => $_POST["establecimiento"], ":fechavisita" => $_POST["fechaVisita"],":fechaviatico" => $_POST["fechaViatico"],
	...
	$sql = "BEGIN  art.hys_prevencionweb.do_guardartarea ( 	:cuit, :establecimiento, TO_DATE(:fechavisita,'DD/MM/YYYY'),
															TO_DATE(:fechaviatico,'DD/MM/YYYY'), :horadesde, :kms, 
															:idpreventor, :idpreventorconsultora, :observaciones,
															:usuario, :idvisita,  :cantvisitas, 'A'); END;";
