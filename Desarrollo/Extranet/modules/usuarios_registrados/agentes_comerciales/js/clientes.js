function abrirVentanaBusquedaEntidad(canal) {
	...
	divWin.load('iframe', '/modules/usuarios_registrados/agentes_comerciales/buscar_entidad.php?c=' + canal, 'Buscar Entidad');
	divWin.show();
}

function cambiaCanal(canal) {
	iframeProcesando.location.href = '/modules/usuarios_registrados/agentes_comerciales/cambia_canal.php?idcanal=' + document.getElementById('canal').value;
}

function cambiaEntidad(entidad) {
	iframeProcesando.location.href = '/modules/usuarios_registrados/agentes_comerciales/cambia_entidad.php?identidad=' + document.getElementById('entidad').value;
}
...
function mostrarCarta(contrato, entidadContrato) {
	...
	window.open('/modules/usuarios_registrados/agentes_comerciales/rc_contratos_activos/mostrar_carta.php?c=' + contrato + '&ec=' + entidadContrato + '&sa=' + valor, 'extranetWindow', 'location=0');
}