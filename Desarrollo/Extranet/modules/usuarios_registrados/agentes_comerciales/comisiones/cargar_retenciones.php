...
$sql =
	"SELECT ¿id?,
		 ...
		 FROM (SELECT 'IB_' || pv_codigo || '_' || lc_id || '_' || il_id id, 'Ingresos Brutos' tipo, INITCAP(pv_descripcion) provincia, il_comprobante comprobante, il_retencion retencion, lc_identidad identidad
				...
				UNION ALL
					 SELECT 'OS_0_' || TO_CHAR(lc_id) || '_0' id, 'Obra Social' tipo, NULL provincia, NULL comprobante, lc_obrasocial retencion, lc_identidad identidad
						 FROM xlc_liqcomision
						WHERE lc_id = :idliquidacion
				UNION ALL
					 SELECT 'G_' || rs_impuesto || '_' || TO_CHAR(lc_id) || '_0' id, tb_descripcion tipo, NULL provincia, rs_numero comprobante, rs_retencion retencion, lc_identidad identidad
...
?>
		...
		<script type="text/javascript">
			with (window.parent.document) {
				getElementById('tituloRetenciones').innerHTML = '<?= $titulo?>';
				getElementById('divContentGridRetenciones').innerHTML = document.getElementById('divContentGrid').innerHTML;
				getElementById('btnExportarRetenciones').style.display = 'inline';
