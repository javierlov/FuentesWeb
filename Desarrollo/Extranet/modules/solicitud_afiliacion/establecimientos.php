	...
	<body style="margin:0;">
		...
		<form action="/modules/solicitud_afiliacion/establecimientos.php" id="formEstablecimientos" method="get" name="formEstablecimientos">
			...
			<div align="left" style="color:#211e1e; font-family:Trebuchet MS; font-size:8pt; margin-bottom:16px; margin-left:8px; margin-top:8px;">
				...
				<label for="nombre" style="margin-left:16px;">Nombre</label>
				<input id="nombre" name="nombre" style="width:88px;" type="text" value="<?= $nombre?>" />
				<label for="domicilio" style="margin-left:16px;">Domicilio</label>
				<input id="domicilio" name="domicilio" style="width:120px;" type="text" value="<?= $domicilio?>" />
				<input class="btnBuscar" style="margin-left:24px; vertical-align:-3px;" type="submit" value="" />
			</div>
			<div align="left" style="margin-bottom:4px;">
				<input class="btnAgregarEstablecimiento" style="margin-left:2px;" type="button" value="" onClick="document.getElementById('iframeProcesando').src = '/modules/solicitud_afiliacion/abrir_ventana_establecimiento.php?idModulo=<?= $_REQUEST["idModulo"]?>&idSolicitud=<?= $_REQUEST["idSolicitud"]?>&id=-1'">
...
if ($_REQUEST["idSolicitud"] == 9) {
	$params[":usualta"] = "W_".substr($_SESSION["usuario"], 0, 18);
	$sql =
		"SELECT ¿se_id?,
						se_id ¿id2?,
						se_id ¿id3?,
						CASE WHEN (LENGTH(se_nroestableci) > 4)
						...
						DECODE(sf_id, NULL, 'btnRGRL', 'btnRGRLOk') ¿buttonclass?
			 FROM ase_solicitudestablecimiento, hys.hsf_solicitudfgrl
			WHERE se_id = sf_idsolicitudestablecimiento(+)
				AND se_usualta = :usualta
				AND se_idsolicitud = :idsolicitud _EXC1_";
}
else {
	$sql =
		"SELECT ¿se_id?,
...
?>
	...
	<script type="text/javascript">
		...
		window.parent.document.getElementById('establecimientos').value = <?= $grilla->recordCount()?>;
