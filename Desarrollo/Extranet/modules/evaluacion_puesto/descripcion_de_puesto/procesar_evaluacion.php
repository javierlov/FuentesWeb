...
try {
	...
	if ($_POST["modo"] == "E") {		// Si terminó de modificar se envía un e-mail..
		if ($_SESSION["idUsuario"] == $_SESSION["idEvaluado"]) {		// Si es el evaluado..
			...
			$row = DBGetQuery($stmt);
			$body = "<html><body>El usuario ".$row["PL_EMPLEADO"]." ha finalizado la descripción de su puesto, <a href='".$link.LOCAL_PATH_DESCRIPCION_PUESTO."'>haga clic aquí</a> para consultar.<br><br>Si el link no funciona pegue esta dirección en su navegador: ".$link.LOCAL_PATH_DESCRIPCION_PUESTO."</body></html>";
		...
		elseif ($esJefe) {		// Si es el jefe..
			...
			$stmt = DBExecSql($conn, $sql, $params);
			$row = DBGetQuery($stmt);
			$body = "<html><body>El jefe del empleado ".$row["PL_EMPLEADO"]." ha finalizado la descripción de su puesto, <a href='".$link.LOCAL_PATH_DESCRIPCION_PUESTO."'>haga clic aquí</a> para consultar.<br><br>Si el link no funciona pegue esta dirección en su navegador: ".$link.LOCAL_PATH_DESCRIPCION_PUESTO."</body></html>";
			SendEmail($body, "Sistema de Descripción de Puesto", "[SDP] Aviso de descripción cargada", array($row["PL_MAIL"]), array(), array(), "H");
		}
		else {		// Si es rrhh..
			$params = array(":id" => $_SESSION["idEvaluado"]);
			...
			$stmt = DBExecSql($conn, $sql, $params);
			$row = DBGetQuery($stmt);
			$body = "<html><body>La carga de la descripción de su puesto ha finalizado, <a href='".$link.LOCAL_PATH_DESCRIPCION_PUESTO."'>haga clic aquí</a> para consultar.<br><br>Si el link no funciona pegue esta dirección en su navegador: ".$link.LOCAL_PATH_DESCRIPCION_PUESTO."</body></html>";
	...
	if ($_POST["modo"] == "N") {		// Si se está notificando se envía un e-mail..
		if ($_SESSION["idUsuario"] == $_SESSION["idEvaluado"]) {		// Si es el evaluado..
			...
			$stmt = DBExecSql($conn, $sql, $params);
			$row = DBGetQuery($stmt);
			$body = "<html><body>El usuario ".$row["PL_EMPLEADO"]." se ha dado por notificado sobre la descripción de su puesto, <a href='".$link.LOCAL_PATH_DESCRIPCION_PUESTO."'>haga clic aquí</a> para consultar.<br><br>Si el link no funciona pegue esta dirección en su navegador: ".$link.LOCAL_PATH_DESCRIPCION_PUESTO."</body></html>";
			...
			$stmt = DBExecSql($conn, $sql, $params);
			$row = DBGetQuery($stmt);
			$body = "<html><body>El jefe del empleado ".$row["PL_EMPLEADO"]." se ha dado por notificado sobre la descripción de su puesto, <a href='".$link.LOCAL_PATH_DESCRIPCION_PUESTO."'>haga clic aquí</a> para consultar.<br><br>Si el link no funciona pegue esta dirección en su navegador: ".$link.LOCAL_PATH_DESCRIPCION_PUESTO."</body></html>";
			SendEmail($body, "Sistema de Descripción de Puesto", "[SDP] Aviso de notificación efectuada", array($row["PL_MAIL"]), array(), array(), "H");
		}
	}
}
