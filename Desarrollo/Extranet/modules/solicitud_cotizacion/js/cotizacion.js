...
function addActividad() {
	with (document) {
		if (getElementById('trActividad2').style.display == '') {
			getElementById('trActividad2').style.display = 'table-row';
			getElementById('ciiu2').focus();
		}
		else
			if (getElementById('trActividad3').style.display == '') {
				getElementById('btnAgregarActividad').style.display = 'none';
				getElementById('trActividad3').style.display = 'table-row';
				getElementById('ciiu3').focus();
			}
	}
}
...
function bloqueaCheckPCP() {
	var cuit = document.getElementById('cuit').value;
	limpiarAvisoPCP();

	if (typeof String.prototype.trim !== 'function') {		// IF agregado para corregir problema en IE8..
	...
	if (cuit[0] == '3') {
		document.getElementById('avisoPCP').innerHTML = 'No puede modificar PCP.';
		document.getElementById('avisoPCP').style.display = 'block';

		document.getElementById('soloPCP').checked = false;
		document.getElementById('soloPCP').disabled = true;
		return false;
	}
...
function calcularTotalesPCP() {
	var elements = document.getElementsByTagName('input');

	var totalAlicuota = 0;
	var totalTrabajadores = 0;
	for (var i=0; i<elements.length; ++i)
		if (elements[i].id.substr(0, 16) == 'trabajadoresPCP_') {
			id = elements[i].id.substr(16);
			document.getElementById('alicuotaPCP_' + id).value = '$ ' + (Number(elements[i].value) * Number(document.getElementById('valorPCP_' + id).value)).toFixed(2);
			totalAlicuota+= Number(document.getElementById('alicuotaPCP_' + id).value.replace('$ ', ''));
			totalTrabajadores+= Number(elements[i].value);

			if (isNaN(document.getElementById('alicuotaPCP_' + id).value.replace('$ ', '')))
				document.getElementById('alicuotaPCP_' + id).value = '';
		}

	document.getElementById('totalAlicuotaPCP').value = '$ ' + totalAlicuota.toFixed(2);
	if (isNaN(document.getElementById('totalAlicuotaPCP').value.replace('$ ', '')))
		document.getElementById('totalAlicuotaPCP').value = '';

	document.getElementById('totalTrabajadoresPCP').value = totalTrabajadores;
	if (isNaN(document.getElementById('totalTrabajadoresPCP').value))
		document.getElementById('totalTrabajadoresPCP').value = '';
}
...
function limpiarAvisoPCP(){	
	with (document) {
		getElementById('avisoPCP').innerHTML = '';
		getElementById('avisoPCP').style.display = 'none';
		getElementById('soloPCP').disabled = false;
	}
}
...
//		getElementById('observaciones').readOnly = lock;
		...
		getElementById('soloPCP').disabled = lock;
		...
		if (lock) {
			...
			for (var i=0; i<elements.length; ++i)
				if (elements[i].id.substr(0, 16) == 'trabajadoresPCP_') {
					elements[i].className = 'pcpCampoGris';
...
function mostrarSoloPCP(mostrar) {
	var display = (mostrar)?'none':'table';

	with (document) {
		getElementById('tableActividad').style.display = display;

		if (getElementById('btnAgregarActividad') != null)
			getElementById('btnAgregarActividad').style.display = display;

		if (getElementById('divResponsabilidadCivil') != null)
			getElementById('divResponsabilidadCivil').style.display = display;

		getElementById('tableTotalesActividad').style.display = display;
		getElementById('tablePeriodo').style.display = display;
		getElementById('tableActividadReal').style.display = display;
		getElementById('tableStatusSrt').style.display = display;
		getElementById('tableStatusBcra').style.display = display;
		getElementById('tableDatosCompetencia').style.display = display;
		getElementById('tableEdadPromedio').style.display = display;

		if (!mostrar)
			if (getElementById('suscribePolizaRC') != null)
				getElementById('suscribePolizaRC').value = 'N';
	}
}
...
function verificarPCP(win) {
	with (win) {
		if (getElementById('cuit').value.substr(0, 1) == '3') {
			getElementById('divPCP').style.display = 'none';
			getElementById('tdSoloPCP').style.display = 'none';

//			if (getElementById('art').value <= 0)
//				getElementById('tdSoloPCP').style.display = 'block';
		}
		else {
			switch (getElementById('statusSrtTmp').value) {
				case '-1':
				case '1':
				case '3':
				case '4':
				case '5':
				case '7':
					getElementById('divPCP').style.display = 'block';
					getElementById('tdSoloPCP').style.display = 'block';
					break;
				default:
					getElementById('divPCP').style.display = 'none';
