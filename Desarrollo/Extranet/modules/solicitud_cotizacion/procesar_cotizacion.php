...
function getIdActividad($codigo) {
	...
	if ($codigo == "")
		...
		$sql =
          ...
          AND CASE WHEN ac_codigo = '950000' THEN NULL ELSE ac_fechabaja END IS NULL";        
//					AND ac_fechabaja IS NULL";
		return valorSql($sql, 0, $params, 0);
	}
}

function getTotalTrabajadoresPCP() {
	$result = 0;

	foreach ($_POST as $key => $value)
		if (substr($key, 0, 16) == "trabajadoresPCP_")
			$result+= intval($value);

	return $result;
}

function getValoresPCP() {
	$result = "";
	...
	foreach ($_POST as $key => $value)
		if (substr($key, 0, 16) == "trabajadoresPCP_") {
			...
			if ($result != "")
				$result.= "|";
			$result.= $arr[1]."_".(($value == "")?"0":intval($value));
		}

	return $result;		//Los datos a devolver son algo parecido a esto: 10_0|11_8|12_7..
}
...
?>
		...
		if (getElementById('descuentoTopeF931').value > 0)
			getElementById('trDescuento').style.display = '';
		else
...
//		getElementById('spanAlicuotaAumento').innerHTML = 'Alícuota ' + getElementById('aumentoTopeF931').value + '% Aumento';
		...
		if (getElementById('aumentoTopeF931').value > 0)
			getElementById('trAumento').style.display = '';
		else
...
try {
	...
	if ($_POST["id"] == "") {		// Es un alta..
		...
		if (($soloPCP != "S") and ($_POST["statusSrtTmp"] < 1))
		...
		// Si es SOLO PCP, seteo por defecto los valores que no se necesitan..
		if ($soloPCP == "S") {
			if (getTotalTrabajadoresPCP() < 1)
				throw new Exception("Debe ingresar al menos un (1) trabajador.");
			$_POST["actividadReal"] = "Servicios de hogares privados que contratan servicio doméstico";
			$_POST["cantidadEstablecimientos"] = 0;
			$_POST["ciiu1"] = "950000";
			$_POST["edadPromedio"] = 35;
			$_POST["masaSalarial1"] = 0;
			$_POST["periodo"] = date("Y/m");
			$_POST["rDatosCompetencia"] = "";
			$_POST["sector"] = 4;
			$_POST["statusBcra"] = -1;
			$_POST["suscribePolizaRC"] = "N";
			$_POST["totalTrabajadores"] = getTotalTrabajadoresPCP();
			$_POST["totalTrabajadores1"] = getTotalTrabajadoresPCP();
			$_POST["zonaGeografica"] = 1;
		}
		...
		if (trim($row["ADVERTENCIA"]) != "") {
			if ($soloPCP != "S")		// Este if lo puse por e-mail de EVila del 22.10.2014..
...
if (valorSql($sql) == "") {
?>
	window.parent.location.href = '/buscar-cotizacion/<?= $nroSol?>/ok';		// insert=ok..
<?
}
else {
?>
	window.parent.location.href = '/cotizacion/<?= $modulo.$id?>/ok';		// insert=ok..
<?
}
?>
</script>